...
2018年3月30日，Google在加州山景城举行了第二届TensorFlow Dev Summit开发者峰会，并宣布正式发布TensorFlow 1.8版本。作为国内的第一批机器学习领域的Google Developers Expert（GDE），笔者有幸获得Google的资助亲临峰会现场，见证了这一具有里程碑式意义的新版本发布。众多新功能的加入和支持展示了TensorFlow的雄心壮志，即时执行（Eager Execution）模式（或称“动态图模式”）在这一版本中终于正式加入，并成为了入门TensorFlow的官方推荐模式。
...
* 主要基于TensorFlow最新的即时执行模式，以便于模型的快速迭代开发。但依然会包含传统的图执行模式与会话机制，代码上尽可能兼容两者；
...
关于本手册的意见和建议，欢迎在 https://discuss.tf.wiki 或图灵社区提交。这是一个开源项目，您的宝贵意见将促进本手册的持续更新。
